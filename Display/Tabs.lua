...
function addonTable.Display.TabsBarMixin:GetFilter(tabData)
  if tabData.invert then
    return function(data)
      return tabData.groups[data.typeInfo.type] ~= false and
        (
      ...
      ) and ((data.typeInfo.type ~= "WHISPER" and data.typeInfo.type ~= "BN_WHISPER") or tabData.whispersTemp[data.typeInfo.player.name] ~= false)
      or (data.typeInfo.type == "ADDON" and tabData.groups["ADDON"] == false and tabData.addons[data.typeInfo.source] ~= false)
    end
  else
    return function(data)
      return tabData.groups[data.typeInfo.type] or
        (data.typeInfo.type == "WHISPER" or data.typeInfo.type == "BN_WHISPER") and tabData.whispersTemp[data.typeInfo.player.name] or
        tabData.channels[data.typeInfo.channel and data.typeInfo.channel.name] or
        data.typeInfo.type == "ADDON" and not tabData.groups["ADDON"] and tabData.addons[data.typeInfo.source]
    end
  end
end
...
function addonTable.Display.TabsBarMixin:RefreshTabs()
  ...
  for index, tabData in ipairs(addonTable.Config.Get(addonTable.Config.Options.WINDOWS)[self.chatFrame:GetID()].tabs) do
    ...
    tabButton.filter = self:GetFilter(tabData)
    ...
    if lastButton == nil then
      tabButton:SetPoint("BOTTOMLEFT", self.chatFrame, "TOPLEFT", 32, -22)
    else
  ...
  if self.chatFrame:GetID() == 1 and addonTable.Config.Get(addonTable.Config.Options.SHOW_COMBAT_LOG) then
    ...
    if lastButton == nil then
      combatLogButton:SetPoint("BOTTOMLEFT", self.chatFrame, "TOPLEFT", 32, -22)
    else
  ...
  if not addonTable.Config.Get(addonTable.Config.Options.LOCKED) then
    local newTab = self.chatFrame.tabsPool:Acquire()
  ...
  self.Tabs = allTabs
  local currentTab = self.chatFrame.tabIndex and math.min(self.chatFrame.tabIndex, #addonTable.Config.Get(addonTable.Config.Options.WINDOWS)[chatFrame:GetID()].tabs) or 1
